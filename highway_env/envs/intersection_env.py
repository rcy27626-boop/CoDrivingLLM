...
class IntersectionEnv(AbstractEnv):
    ...
    #     # 4: 'SLOWER'
    # }
    # ACTIONS_INDEXES = {v: k for k, v in ACTIONS_ALL.items()}
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "duration": 50,  # [s]
            "destination": False,  # TODO: all the destination of vehicles
            "controlled_vehicles": 1,
            "initial_vehicle_count": 10,
            "spawn_probability": 0.6,
            "screen_width": 600,
            "screen_height": 600,
            "centering_position": [2, 0.6],
            "scaling": 3,
            "collision_reward": -5,
            "high_speed_reward": 1,
            "arrived_reward": 1,
    ...
    def _is_terminal(self) -> bool:
        ...
        return any(vehicle.crashed for vehicle in self.controlled_vehicles) \
               ...
               or self.steps >= self.config["duration"] * self.config["policy_frequency"]-1 \
    ...
    #     # done: if the episode is over: is_terminal or crashed or timeout or offroad
    #     # info['has_arrived'] : if the controlled_vehicle has arrived
    #     return obs, reward, done, info
    ...
    def _make_road(self) -> None:
        ...
        for corner in range(4):
            ...
            net.add_lane("o" + str(corner), "ir" + str(corner),
                         StraightLane(start, end, line_types=[s, c], priority=priority, speed_limit=5))
            # Right turn
            ...
            net.add_lane("ir" + str(corner), "il" + str((corner - 1) % 4),
                         CircularLane(r_center, right_turn_radius, angle + np.radians(180), angle + np.radians(270),
                                      line_types=[n, c], priority=priority, speed_limit=5))
            # Left turn
            ...
            net.add_lane("ir" + str(corner), "il" + str((corner + 1) % 4),
                         CircularLane(l_center, left_turn_radius, angle + np.radians(0), angle + np.radians(-90),
                                      clockwise=False, line_types=[n, n], priority=priority - 1, speed_limit=5))
            # Straight
            start = rotation @ np.array([lane_width / 2, outer_distance])
            end = rotation @ np.array([lane_width / 2, -outer_distance])
            net.add_lane("ir" + str(corner), "il" + str((corner + 2) % 4),
                         StraightLane(start, end, line_types=[s, n], priority=priority, speed_limit=5))
            # Exit
            ...
            net.add_lane("il" + str((corner - 1) % 4), "o" + str((corner - 1) % 4),
                         StraightLane(end, start, line_types=[n, c], priority=priority, speed_limit=5))
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            # destination = self.config["destination"] or "o{}".format(3 - ego_id % 4)
            destination = self.config["destination"] or "o" + str(self.np_random.randint(1, 4))
            while destination == "o{}".format(ego_id % 4):
                destination = "o" + str(self.np_random.randint(1, 4))
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(60 + 5*self.np_random.normal(1), 0),
                             speed=5,
                             heading=ego_lane.heading_at(60))   # 初始速度5m/s
            try:
